...
const itemSub = document.querySelectorAll('#navHead li i');
    ...
    for (const prog of boxPrograma) {
        prog.addEventListener("click", function(e){
            ...
            for (const ulP of ulPrograma) {
                ulP.parentElement.classList.remove('open');
                ulP.parentElement.classList.add('close');
                ulP.classList.remove('visible'); 
            }
            const p = prog.parentElement.getElementsByTagName('ul');prog.parentElement.classList.add('open');
            prog.parentElement.classList.remove('close');  
...
const selIdiomas = document.querySelectorAll('.select-idiomas');

if(idioma){
    for (const idi of idioma) {
        idi.addEventListener("click", function(e){
            idi.classList.add('visible')
        });
    }
}

const verCharla = document.querySelectorAll('.item-charla .ver-mas');
const cerrarCharla = document.querySelectorAll('.item-charla .cerrar-charla');
if(verCharla){
    for (const vc of verCharla) {
        vc.addEventListener("click", function(e){
            e.preventDefault();
            vc.parentElement.classList.toggle('visible');
        });
    }
}
if(cerrarCharla){
    for (const cc of cerrarCharla) {
        cc.addEventListener("click", function(e){
            e.preventDefault();
            cc.parentElement.classList.toggle('visible');
        });
    }
}

document.addEventListener("click", (evt) => {
    const selIdioma = document.getElementsByClassName('idioma');
    let targetElement = evt.target; // clicked element

    do {
        if (targetElement == selIdioma[0]) {
            return;
        }
        // Go up the DOM.
    ...
    selIdioma[0].classList.remove('visible');
});
